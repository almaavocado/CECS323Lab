...
SELECT COALESCE(state, customerName) as Customer,
             customerName,
             state
FROM   CUSTOMERS
ORDER BY  Customer, customerName, contactLastName, contactFirstName;



SELECT C.CUSTOMERNAME, C.STATE, O.STATE FROM CUSTOMERS AS C INNER JOIN employees e on e.employeeNumber = C.salesRepEmployeeNumber
INNER JOIN offices o on e.officeCode = o.officeCode
WHERE COALESCE(C.STATE, 'CA') <> coalesce(O.STATE, 'CA')
ORDER BY customerName asc;


/*
 Select customer name, order date, shipped date, quantity ordered, product line, product name
 for all orders made in May of 2013  and shipped in some other month.  Order by customer name and order date (17)
 */

SELECT DISTINCT customerName FROM (customers c JOIN orders o ON c.customerNumber = o.customerNumber
    JOIN orderdetails od on o.orderNumber = od.orderNumber JOIN products p on od.productCode = p.productCode)
WHERE p.productName = 'Pont Yacht';

/*
 List the city, state, and country of each office that has an employee working at that office and
 the employee is the sales representative for a customer who has ever bought ‘1928 Mercedes-Benz SSK’.  Order by the office code (5)
 */
SELECT DISTINCT o3.city, O3.state, o3.country, o3.officeCode FROM CUSTOMERS AS C
...
INNER JOIN OFFICES o3 on e.OFFICECODE = o3.officeCode
      where P.PRODUCTNAME = '1928 Mercedes-Benz SSK'
ORDER BY o3.officeCode asc;
/*
Select customer name, order date, shipped date, quantity ordered, product line, product name
for all orders made in May of 2013  and shipped in some other month.  Order by customer name and order date (17)
*/

SELECT C.CUSTOMERNAME, O.ORDERDATE, O.shippedDate, O2.quantityOrdered, p.productLine, p.productName FROM CUSTOMERS C
INNER JOIN ORDERS as O on C.customerNumber = O.customerNumber
INNER JOIN ORDERDETAILS O2 on O.ORDERNUMBER = O2.ORDERNUMBER
INNER JOIN PRODUCTS P on O2.PRODUCTCODE = P.PRODUCTCODE
where MONTH(orderDate) = 5 and YEAR(orderDate) = 2013 and MONTH(shippedDate) != 5
ORDER BY customerName, orderDate asc;

/*

Select customer name, order number, order date, and status on all orders in which at least one of the products ordered has a quantity ordered > the quantity in stock for that product.  List each order no more than once.  Order by the customer name and the order number.  (28)
*/



/*
 List all customers who didn't order in 2015.  List them in ascending order by customer name.  (78)
 */
...
SELECT contactFirstName, contactLastName, 'Customer' as "Contact Type" FROM CUSTOMERS
UNION
SELECT firstName, lastName, 'employee' as "Contact Type" FROM employees
ORDER BY 2,1 ASC;


 /*

  List all people that we deal with (employees and customer contacts).
  Display first name, last name, company name (or employee) order by last name, then first name.
  Order by last name, then the first name.  (145)

  */

--3
SELECT DISTINCT E.lastName, E.firstName, E.employeeNumber
  FROM employees E
...
--4
SELECT country, state, 'Customer' AS Category FROM customers
WHERE state IS NOT NULL
EXCEPT
SELECT country, state, 'Customer' AS Category FROM offices
WHERE state IS NOT NULL
UNION
SELECT country, state, 'Office' AS Category FROM offices
WHERE state IS NOT NULL
EXCEPT
SELECT country, state, 'Office' AS Category FROM customers
WHERE state IS NOT NULL
UNION
SELECT c.country, c.state, 'Both' AS Category FROM customers c
INNER JOIN offices o ON c.country = o.country AND c.state = o.state
...
EXCEPT
SELECT c1.customername, c1.country, c1.state FROM customers c1
INNER JOIN customers c2 ON c1.country = c2.country AND COALESCE(c1.state,'N/A') = COALESCE(c2.state,'N/A') AND c1.customernumber != c2.customernumber
ORDER BY customername;

--1
SELECT C1.CUSTOMERNUMBER
FROM CUSTOMERS C1 INNER JOIN payments p
   ON C1.customerNumber = p.customerNumber
EXCEPT
SELECT C.CUSTOMERNUMBER
FROM CUSTOMERS C INNER JOIN PAYMENTS P
   ON C.CUSTOMERNUMBER = P.CUSTOMERNUMBER
WHERE P.PAYMENTDATE IN (
   SELECT P2.PAYMENTDATE
   FROM CUSTOMERS C2 INNER JOIN PAYMENTS P2
       ON C2.CUSTOMERNUMBER = P2.CUSTOMERNUMBER
   WHERE C.CUSTOMERNUMBER != C2.CUSTOMERNUMBER)
...
--2 Recursion
SELECT e1.employeeNumber, e1.lastName, e1.firstName FROM employees e1
INNER JOIN employees e2 ON e1.reportsTo = e2.employeeNumber
WHERE e1.jobTitle = 'Sales Rep'
ORDER BY e1.employeeNumber, e1.lastName, e1.firstName;
...
--3
SELECT c1.customername, c1.city, c1.state, c1.country, c1.salesRepEmployeeNumber,
       c2.CUSTOMERNAME, c2.CITY, c2.STATE, c2.COUNTRY, c2.SALESREPEMPLOYEENUMBER
FROM customers c1
INNER JOIN customers c2 ON c1.country = c2.country
WHERE c1.city = c2.CITY AND c1.country = c2.COUNTRY
  AND c1.salesRepEmployeeNumber = c2.SALESREPEMPLOYEENUMBER AND c1.customerNumber < c2.customerNumber;



/*
-->Report out the last name, the first name, and the type of employee for three classes of employee: those who
--don’t report to anyone, those who report to anyone who doesn’t report to anyone, and those who report to
--those who report to anyone who doesn’t report to anyone.  The type of employee will be ‘Big Kahuna’ in the
--first case, ‘1st Level Down’ in the second case, and ‘2nd Level Down’ in the third case.  Order by the last name,
then the first name.  Returns 7 rows: 1 in the first type, 2 in the second, and 4 in the third type.  Broad hint,
you are going to have to do two unions for this.
*/


--4
SELECT E.lastName, E.firstName, 'Big Kahuna' type
FROM EMPLOYEES E
WHERE E.REPORTSTO is NULL
UNION
SELECT E.lastName, E.firstName, 'First Level Down' type
FROM EMPLOYEES E
INNER JOIN EMPLOYEES VP ON E.REPORTSTO = VP.employeeNumber
WHERE VP.REPORTSTO is NULL
UNION
SELECT E.lastName, E.firstName, 'Second Level Down' type
FROM EMPLOYEES E
...
--5 Recursion
SELECT o1.orderNumber AS "Order 1", o2.orderNumber AS "Order 2", o1.ORDERDATE, o1.SHIPPEDDATE, o1.STATUS
   FROM orders o1 INNER JOIN orders o2
   ON o1.orderDate = o2.orderDate
   AND o1.shippedDate = o2.shippedDate
